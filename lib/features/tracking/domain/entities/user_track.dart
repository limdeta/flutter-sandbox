...
class UserTrack {
  ...
  /// Дополнительные метаданные трека
  ...
  /// Создает UserTrack из одного CompactTrack (без сегментации)
  ...
  /// Создает UserTrack из множественных сегментов
  factory UserTrack.fromSegments({
    ...
    final nonEmptySegments = segments.where((s) => !s.isEmpty).toList();
  ...
  /// Проверяет активен ли трек (не завершен)
  bool get isActive => status.isActive;

  /// Проверяет завершен ли трек
  bool get isCompleted => endTime != null;

  /// Проверяет пустой ли трек
  bool get isEmpty => segments.isEmpty || totalPoints == 0;

  /// Проверяет не пустой ли трек
  bool get isNotEmpty => !isEmpty;

  /// Количество сегментов в треке
  int get segmentCount => segments.length;

  /// Получает объединенный трек всех сегментов (используйте осторожно для больших треков!)
  CompactTrack get fullTrack => CompactTrack.merge(segments);

  /// Получает сегмент по индексу
  ...
  /// Получает сегменты в указанном временном диапазоне
  ...
  /// Вычисляет статистику по сегментам
