...
import 'package:tauzero/features/route/data/mappers/route_mapper.dart';

/// Сервис для создания фиксированного набора торговых точек
/// 
/// Создает торговые точки ОДИН раз, затем они используются в маршрутах по ссылкам.
/// Это решает проблему дублирования и UNIQUE constraint failures.
class TradingPointsFixtureService {
  
  /// Создает базовый набор торговых точек для всех маршрутов
  ...
  Future<void> _saveTradingPoint(TradingPoint point) async {
    ...
    final companion = RouteMapper.tradingPointToDatabase(point);
