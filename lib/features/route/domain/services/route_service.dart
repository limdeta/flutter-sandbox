...
class RouteService {
  ...
  Route createRoute({
    ...
    required List<IPointOfInterest> pointsOfInterest,
    String? description,
    DateTime? startTime,
  }) {
    return Route(
  ...
  Route startRoute(Route route) {
    ...
    IPointOfInterest? firstUncompletedPoint;
    ...
    final updatedPoints = <IPointOfInterest>[];
    for (int i = 0; i < route.pointsOfInterest.length; i++) {
  ...
  bool _isPointCompletedAtTime(IPointOfInterest point, DateTime time) {
...
class RouteTimelineFrame {
  ...
  final IPointOfInterest? currentPoint;
  final List<IPointOfInterest> completedPoints;
